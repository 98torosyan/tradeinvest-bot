...
Two ways this can end, depending on what secrets are set:
  1. If GH_PAT (a GitHub Personal Access Token with the "repo" scope,
     ...
     secret -- fully automatic, nothing for you to do.
  2. Otherwise, the new token is sent to you on Telegram so you can
     paste it into the IG_ACCESS_TOKEN secret by hand (Settings ->
...
def main():
    ...
    if GH_PAT and GITHUB_REPOSITORY:
        ...
        print("[refresh_ig_token] GH_PAT not set -- sent new token via Telegram for manual update")
